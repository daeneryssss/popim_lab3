...
const replaceSidebarIcons = (code) => {
    ...
    while ((match = matchIconAnnotation.exec(code)) !== null) {
        if (!iconNames.includes(match[1])) {
            ...
            console.warn(`Icon ${match[1]} not found, see: https://lucide.dev/icons/`);
...
export const invalidate = (server) => {
    const sidebarModule = server.environments.ssr.moduleGraph.getModuleById(virtualModuleId);
    if (!sidebarModule)
        return;
...
export const viteSidebarPlugin = (getConfig) => {
    return {
        ...
        async load(id) {
            ...
            await writePluginDebugCode(config.__meta.rootDir, "sidebar-plugin", code, "json");
            return code;
        },

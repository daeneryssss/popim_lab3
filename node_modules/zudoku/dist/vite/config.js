...
import { loadConfigFromFile, mergeConfig, loadEnv as viteLoadEnv, } from "vite";
...
const getDocsConfigFiles = (docsConfig, baseDir) => {
    if (!docsConfig)
        return [];
...
function loadEnv(configEnv, rootDir) {
    ...
    const publicEnv = Object.entries(process.env).reduce((val, [key]) => {
...
let envPrefix;
let publicEnv;
export const getStandaloneConfig = (rootDir) => ({
    __meta: {
        rootDir,
...
export async function loadZudokuConfig(configEnv, rootDir, forceReload) {
    ...
    try {
        ...
        logger.info(colors.yellow(`loaded config file `) +
            colors.dim(config.__meta.configPath), { timestamp: true });
        return { config, envPrefix, publicEnv };
    }
    catch (error) {
                ...
                plugins: [
                    tailwindcss({
                        ...
                        content: [
                            ...
                            ...config.__meta.dependencies.map((dep) => `${path.dirname(config.__meta.configPath)}/${dep}`),
